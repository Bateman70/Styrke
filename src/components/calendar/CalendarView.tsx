...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const startDate = startOfWeek(monthStart, { weekStartsOn: 1 }); // Mandag
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Kalendervisning for styrke og løping</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="ml-2 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-medium text-xs transition-all shadow-md shadow-blue-600/20 flex items-center space-x-1.5"
          >
            <Sparkles className="w-4 h-4" />
            <span className="hidden sm:inline">Generer ukesplan</span>
          </button>
        </div>
                    ...
                    {dayLogs.map((log) => {
                      ...
                      if (log.type === 'okt-a') {
                        return (
                          <div
                            key={log.id}
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold flex items-center justify-between truncate ${
                              isCompleted
                          ...
                          >
                            ...
                            {isCompleted ? <CheckCircle2 className="w-3 h-3 text-emerald-400 shrink-0 ml-1" /> : <Clock className="w-3 h-3 text-emerald-600 shrink-0 ml-1" />}
                          </div>
                        );
                      }
                          ...
                          <div
                            ...
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold flex items-center justify-between truncate ${
                              isCompleted
                                ...
                                : 'bg-indigo-950/20 text-indigo-400 border border-indigo-900/50'
                            }`}
                          >
                            <span className="truncate">Økt B</span>
                            {isCompleted ? <CheckCircle2 className="w-3 h-3 text-indigo-400 shrink-0 ml-1" /> : <Clock className="w-3 h-3 text-indigo-600 shrink-0 ml-1" />}
                          </div>
                        );
                      }
                      ...
                      if (log.type === 'lop') {
                        return (
                          <div
                            key={log.id}
                            className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-orange-950/50 text-orange-300 border border-orange-800/50 flex items-center justify-between truncate"
                          >
                            <span className="truncate">{log.runDetails?.distanceKm || ''} km Løp</span>
                            <Flame className="w-3 h-3 text-orange-400 shrink-0 ml-1" />
                          </div>
                        );
                      }
          ...
          <div>
            ...
            <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">Planlagte og utførte aktiviteter</p>
              </div>
            </div>
                ...
                selectedDayLogs.map((log) => {
                      ...
                      >
                        ...
                        <div className="mt-4 flex items-center justify-between pt-2 border-t border-slate-800/60">
                          ...
                          >
                            ...
                            <span>{isCompleted ? 'Vis / Rediger' : 'Start Økt'}</span>
                          </button>
                        </div>
                      </div>
                    );
                  ...
                  if (log.type === 'lop') {
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="px-2 py-0.5 rounded text-xs font-bold bg-orange-500/20 text-orange-400 flex items-center space-x-1">
                        ...
                        {log.runDetails?.notes && (
                          <p className="text-xs text-orange-300/80 italic mt-2">
                            "{log.runDetails.notes}"
                          </p>
                        )}
